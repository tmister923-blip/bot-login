...
class DiscordBotDashboard {
    ...
    switchTab(tabName) {
        ...
        const subtitles = {
            ...
            music: 'Control music playback with Lavalink integration',
            auth: 'Authenticate with Discord user account tokens',
        ...
        if (tabName === 'stickers') {
            ...
            this.populateMusicServerDropdown();
    ...
    setupMusicPlayer() {
        ...
        // Music panel buttons
        const createPanelBtn = document.getElementById('createMusicPanelBtn');
        if (createPanelBtn) {
            createPanelBtn.addEventListener('click', () => this.createMusicPanel());
        }

        const updatePanelBtn = document.getElementById('updateMusicPanelBtn');
        if (updatePanelBtn) {
            updatePanelBtn.addEventListener('click', () => this.updateMusicPanel());
        }

        const deletePanelBtn = document.getElementById('deleteMusicPanelBtn');
        if (deletePanelBtn) {
            deletePanelBtn.addEventListener('click', () => this.deleteMusicPanel());
        }
    ...
    async connectLavalink() {
        ...
        // First test the Lavalink connection
        try {
            this.showStatus('Testing Lavalink connection...', 'info');
            
            const testResponse = await fetch('/api/music/test-lavalink', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!testData.success) {
                this.showStatus(`❌ Lavalink test failed: ${testData.error}`, 'error');
                return;
            }

            this.showStatus('✅ Lavalink connection test successful!', 'success');
            
            // Show stats if available
            if (testData.stats) {
                this.showStatus(`📊 Lavalink Stats: ${testData.stats.players} players, ${testData.stats.playingPlayers} playing`, 'success');
            }

            // Now connect to the bot
            const connectResponse = await fetch('/api/music/connect-lavalink', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (connectData.success) {
                ...
                this.showStatus('✅ Bot connected to Lavalink successfully!', 'success');
                this.updateLavalinkStatus('Connected');
                
                // Show user ID input after successful connection
                this.showUserIdInput();
            } else {
                this.showStatus(`❌ Bot connection failed: ${connectData.error}`, 'error');
            }
        } catch (error) {
    ...
    showUserIdInput() {
        const userIdInput = document.getElementById('musicUserId');
        const userIdGroup = userIdInput.closest('.form-group');
        
        if (userIdGroup) {
            userIdGroup.style.display = 'block';
            userIdGroup.style.animation = 'fadeIn 0.5s ease-in';
        }
        
        this.showStatus('✅ Please enter your Discord User ID to continue', 'success');
    }
    ...
    async playTrack(url, title, author, thumbnail) {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.showStatus(`🎵 Now playing: ${title} in ${data.voiceChannel}`, 'success');
            } else {
    ...
    async nextTrack() {
        ...
        try {
            const response = await fetch('/api/music/skip', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    populateMusicServerDropdown() {
        console.log('🎵 Populating music server dropdown...');
        const musicServerSelect = document.getElementById('musicServerSelect');
        
        if (!musicServerSelect) {
            console.error('Music server select not found');
            return;
        }

        // Clear existing options
        musicServerSelect.innerHTML = '<option value="">Select a server...</option>';

        // Add guilds to dropdown
        if (this.guilds && this.guilds.length > 0) {
            this.guilds.forEach(guild => {
                const option = document.createElement('option');
                option.value = guild.id;
                option.textContent = guild.name;
                musicServerSelect.appendChild(option);
            });
            console.log(`✅ Added ${this.guilds.length} servers to music dropdown`);
        } else {
            console.log('⚠️ No guilds available for music dropdown');
            musicServerSelect.innerHTML = '<option value="">No servers available</option>';
        }
    }

    // Music Panel Functions
    async createMusicPanel() {
        const guildId = document.getElementById('musicServerSelect').value;
        const channelId = document.getElementById('musicPanelChannelId').value;

        if (!guildId || !channelId) {
            this.showStatus('Please select a server and enter a channel ID', 'error');
            return;
        }

        try {
            const response = await fetch('/api/music/create-panel', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (data.success) {
                this.showStatus('✅ Music panel created successfully!', 'success');
                this.musicPanelMessageId = data.messageId;
                this.musicPanelChannelId = channelId;
            } else {
                this.showStatus(`❌ Failed to create music panel: ${data.error}`, 'error');
            }
        } catch (error) {
            console.error('Create music panel error:', error);
            this.showStatus('❌ Failed to create music panel', 'error');
        }
    }

    async updateMusicPanel() {
        if (!this.musicPanelMessageId || !this.musicPanelChannelId) {
            return;
        }

        try {
            const response = await fetch('/api/music/update-panel', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    channelId: this.musicPanelChannelId,
                    messageId: this.musicPanelMessageId
                })
            });

            const data = await response.json();
            if (!data.success) {
                console.error('Failed to update music panel:', data.error);
            }
        } catch (error) {
            console.error('Update music panel error:', error);
        }
    }

    async deleteMusicPanel() {
        if (!this.musicPanelMessageId || !this.musicPanelChannelId) {
            return;
        }

        try {
            const response = await fetch('/api/music/delete-panel', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            const data = await response.json();
            if (data.success) {
                this.musicPanelMessageId = null;
                this.musicPanelChannelId = null;
                this.showStatus('🗑️ Music panel deleted', 'info');
            }
        } catch (error) {
            console.error('Delete music panel error:', error);
        }
    }
