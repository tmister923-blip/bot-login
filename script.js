            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (canvasResponse.ok) {
                        ...
                        // Test drawn shapes functionality
                        showStatus('Testing drawn shapes rendering...', 'info');
                        try {
                            const emojiResponse = await fetch('/api/test-emoji');
                            if (emojiResponse.ok) {
                                showStatus('✅ Drawn shapes rendering working!', 'success');
                            } else {
                                showStatus('❌ Drawn shapes test failed', 'error');
                            }
                        } catch (emojiError) {
                            showStatus('❌ Drawn shapes test failed: ' + emojiError.message, 'error');
                        }
                    } else {
